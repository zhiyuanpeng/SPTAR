...
@Version :   1.0
@Desc    :   copy from contriever, only change the model name to dragon following:
https://huggingface.co/facebook/dragon-roberta-query-encoder
https://huggingface.co/facebook/dragon-roberta-context-encoder
'''
...
model_name = "tasb"
checkpoint = join(data_dir, "checkpoints")
...
query_state_dict = join(model_save_path, "query_model_state_dict.pt")
doc_state_dict = join(model_save_path, "doc_model_state_dict.pt")

# model = DRES(models.SentenceBERT(checkpoint), batch_size=256, corpus_chunk_size=100000)
model = DRES(models.DPR((query_model_name, doc_model_name), query_state_dict=query_state_dict, doc_state_dict=doc_state_dict, share=True), batch_size=256, corpus_chunk_size=30000)
# model = DRES(models.DPR((checkpoint,checkpoint)), batch_size=256, corpus_chunk_size=50000)
retriever = EvaluateRetrieval(model, k_values=[1,3,5,10,100,300,500,1000], score_function="dot")
...
# uncessary as mrr will sort, trev will sort too as comment this have no influence on results
# for q, r in results.items():
#     results[q] = dict(sorted(r.items(), key=lambda x: x[1], reverse=True))
