...
model_save_path = os.path.join(save_dir, "train", "output", args.exp_name, str(args.train_num), "{}-{}-{}".format(model_name, args.dpr_v, args.dataset_name))
...
logging.info(f"Loading Cross-Encoder Model from {model_save_path}")
cross_encoder_model = CrossEncoder(model_save_path)
reranker = Rerank(cross_encoder_model, batch_size=128)
#### Retrieve dense results (format of results is identical to qrels)
start_time = time()
results = reranker.rerank(corpus, queries, results, top_k=1000)
end_time = time()
