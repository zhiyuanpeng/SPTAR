...
@Version :   1.0
@Desc    :   copy from dpr_eval, change it to bm25ce
'''
...
def multirun(args):
    ...
    for exp in arg_dict["exp_names"]:
        ...
        train_command = ["python", "-m", "zhiyuan.retriever.dpr_dragon.train.train_dragon", "--dataset_name", f"{arg_dict['dataset_name']}", "--train_num", f"{arg_dict['train_num']}", "--weak_num", f"{arg_dict['weak_num']}", "--exp_name", exp]
        # for test
        print(" ".join(train_command))
        print(f"GPU {arg_dict['gpu_id']} Training: {arg_dict['dataset_name']} on {exp}")
        subprocess.call(train_command)
        eval_command = ["python", "-m", "zhiyuan.retriever.dpr_dragon.eval.evaluate_dragon", "--dataset_name", f"{arg_dict['dataset_name']}", "--train_num", f"{arg_dict['train_num']}", "--exp_name", exp, "--dpr_v", arg_dict["version"]]
        # for test
